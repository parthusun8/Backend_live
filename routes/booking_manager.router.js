...
BookingRouter.get('/getPlayers', async (req, res) => {
    try{
        ...
        const result = await Player.findOne({TOURNAMENT_ID : tournament_id, CAPTAIN : captain_id});
        console.log(result);
        if(result){
            const result1 = {
...
BookingRouter.post('/addTeamPlayers', async (req, res) => {
    try{
        ...
        await Player.findOneAndUpdate({TOURNAMENT_ID : req.body.TOURNAMENT_ID, CAPTAIN : req.body.CAPTAIN}, {$push : {PLAYERS : player}});
...
BookingRouter.post('/addSubstitute', async (req, res) => {
    try{
        ...
        await Player.findOneAndUpdate({TOURNAMENT_ID : req.body.TOURNAMENT_ID, CAPTAIN : req.body.CAPTAIN}, {$push : {SUBSTITUTE : subs}});
