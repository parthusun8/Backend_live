        ...
        matchesmodel.findOne(
          ...
          function (error, d) {
            if (error) {
              ...
              res.render("tourna_fixture", {
                TOURNEY_ID: req.query.TOURNAMENT_ID,
                no_of_bracs: result.NO_OF_KNOCKOUT_ROUNDS,
              });
            }
          }
        );
    ...
    tournamentModel.findOne(
      ...
      function (err, result) {
        ...
        if (result) {
          ...
          res.setHeader(
            "Content-Disposition",
            "attachment; filename=details.csv"
          );
    ...
    if (usrresult && r1) {
      if (r1) {
        ...
        for (let i = 0; r1[i]; i++) {
          ...
          console.log(curDate.getTime() > end_date.getTime());
          if (
            curDate.getTime() < end_date.getTime() &&
            usrresult.HOSTED_TOURNAMENTS.includes(r1[i].TOURNAMENT_ID)
          ) {
            console.log(r1[i]);
            r2.push(r1[i]);
          }
  ...
  tournamentModel.findOne(
    ...
    function (error, result) {
      ...
      if (result) {
        ...
        const d1 = new Date(
          new Date(starttimestamp).getTime() - regclosesbefore * 60 * 60 * 1000
        ).getTime();
      ...
      if (result.CURRENT_TOURNAMENTS.length == 0) {
        ...
        for (let i = 0; i < result.CURRENT_TOURNAMENTS.length; i++) {
          try {
            ...
            console.log("EK TOURNAMENT HAI YEH   " + tournament);
            if (tournament) {
              r1.push(tournament);
            }
          } catch (error) {
      ...
      if (result) {
        if (req.body.set == "1") {
          matchesmodel.updateOne(
            ...
            {
              $set: {
                "MATCHES.$[elem].PLAYER1_SCORE.set1": parseInt(
                  req.body.PLAYER_1_SCORE
                ),
                "MATCHES.$[elem].PLAYER2_SCORE.set1": parseInt(
                  req.body.PLAYER_2_SCORE
                ),
              },
            },
            {
          ...
          matchesmodel.updateOne(
            ...
            {
              $set: {
                "MATCHES.$[elem].PLAYER1_SCORE.set2": parseInt(
                  req.body.PLAYER_1_SCORE
                ),
                "MATCHES.$[elem].PLAYER2_SCORE.set2": parseInt(
                  req.body.PLAYER_2_SCORE
                ),
              },
            },
          ...
          matchesmodel.updateOne(
            ...
            {
              $set: {
                "MATCHES.$[elem].PLAYER1_SCORE.set3": parseInt(
                  req.body.PLAYER_1_SCORE
                ),
                "MATCHES.$[elem].PLAYER2_SCORE.set3": parseInt(
                  req.body.PLAYER_2_SCORE
                ),
              },
            },
    ...
    function (error, result) {
      if (error) {
        ...
        if (result.MATCHES.length == 0) {
          ...
          result.MATCHES[
            parseInt(req.query.MATCHID.split(" ")[1]) - 1
          ].MATCHID = req.query.MATCHID.split(" ")[1];
          result.MATCHES[
            parseInt(req.query.MATCHID.split(" ")[1]) - 1
          ...
          if (req.query.TOURNAMENT_ID[0] == "B") {
            res.render(
              "match_view",
              result.MATCHES[parseInt(req.query.MATCHID.split(" ")[1]) - 1]
            );
          } else if (req.query.TOURNAMENT_ID[0] == "T") {
            res.render(
              "table_tennis_match_view",
              result.MATCHES[parseInt(req.query.MATCHID.split(" ")[1]) - 1]
            );
          }
        }
                ...
                if (ress.MATCHES.length - 2 == matchid) {
                  ...
                  if (twon_update) {
                    ...
                    if (ress.MATCHES[matchid].NEXT_MATCH_PLAYER_SPOT == 0) {
                      matchesmodel.updateOne(
                        ...
                        },
                        {
                          arrayFilters: [
                            {
                              "elem.MATCHID":
                                ress.MATCHES[matchid].NEXT_MATCH_ID,
                            },
                          ],
                        },
                        function (e, result2) {
                      ...
                      ress.MATCHES[matchid].NEXT_MATCH_PLAYER_SPOT == 1
                    ) {
                      matchesmodel.updateOne(
                        {
                          TOURNAMENT_ID: req.body.TOURNAMENT_ID,
                  ...
                  if (ress.MATCHES[matchid].NEXT_MATCH_PLAYER_SPOT == 0) {
                    matchesmodel.updateOne(
                      ...
                      },
                      {
                        arrayFilters: [
                          {
                            "elem.MATCHID": ress.MATCHES[matchid].NEXT_MATCH_ID,
                          },
                        ],
                      },
                      function (e, result2) {
    ...
    async function (error, result) {
      if (error) {
        ...
        if (result.MATCHES.length - 2 == matchid) {
          ...
          var LOSER = "";
          if (
            result.MATCHES[matchid].PLAYER1 == "Not Booked" ||
            result.MATCHES[matchid].PLAYER1 == "Not Yet Assigned"
          ) {
            WINNER = result.MATCHES[matchid].PLAYER2;
          } else if (
            result.MATCHES[matchid].PLAYER2 == "Not Booked" ||
            result.MATCHES[matchid].PLAYER2 == "Not Yet Assigned"
          ) {
            ...
            if (
              result.MATCHES[matchid].PLAYER1_SCORE.set1 >
              result.MATCHES[matchid].PLAYER2_SCORE.set1
            ) {
              set1 = 1;
            } else {
              set1 = 0;
            ...
            if (
              result.MATCHES[matchid].PLAYER1_SCORE.set2 >
              result.MATCHES[matchid].PLAYER2_SCORE.set2
            ) {
              set2 = 1;
            } else {
              set2 = 0;
            ...
            if (
              result.MATCHES[matchid].PLAYER1_SCORE.set3 >
              result.MATCHES[matchid].PLAYER2_SCORE.set3
            ) {
              set3 = 1;
            } else {
              set3 = 0;
            ...
            if (req.query.TOURNAMENT_ID[0] == "T") {
              pl1sum +=
                result.MATCHES[matchid].PLAYER1_SCORE.set4 +
                result.MATCHES[matchid].PLAYER1_SCORE.set5;
              pl2sum +=
                result.MATCHES[matchid].PLAYER2_SCORE.set4 +
                result.MATCHES[matchid].PLAYER2_SCORE.set5;
            }
            if (pl1sum > pl2sum) {
              WINNER = result.MATCHES[matchid].PLAYER1;
          ...
          if (winner_name) {
            matchesmodel.findOneAndUpdate(
              ...
              {
                arrayFilters: [
                  { "elem.MATCHID": result.MATCHES[matchid].MATCHID },
                ],
              },
              function (error, d) {
                if (error) {
                ...
                if (response) {
                  USER.updateOne(
                    ...
                    function (er, re) {
                      ...
                      if (re) {
                        if (
                          result.MATCHES[matchid].NEXT_MATCH_PLAYER_SPOT == 0
                        ) {
                          ...
                          matchesmodel.updateOne(
                            ...
                            },
                            {
                              arrayFilters: [
                                {
                                  "elem.MATCHID":
                                    result.MATCHES[matchid].NEXT_MATCH_ID,
                                },
                                {
                                  "elem2.MATCHID":
                                    result.MATCHES[matchid].MATCHID,
                                },
                              ],
                            },
                            function (error, result2) {
          ...
          matchesmodel.findOne(
            ...
            async function (error, result4) {
              if (error) {
                ...
                var LOSER_ID = "";
                if (
                  result4.MATCHES[matchid].PLAYER1_SCORE.set1 >
                  result4.MATCHES[matchid].PLAYER2_SCORE.set1
                ) {
                  set1 = 1;
                } else {
                  set1 = 0;
                ...
                if (
                  result4.MATCHES[matchid].PLAYER1_SCORE.set2 >
                  result4.MATCHES[matchid].PLAYER2_SCORE.set2
                ) {
                  set2 = 1;
                } else {
                  set2 = 0;
                ...
                if (
                  result4.MATCHES[matchid].PLAYER1_SCORE.set3 >
                  result4.MATCHES[matchid].PLAYER2_SCORE.set3
                ) {
                  set3 = 1;
                } else {
                  set3 = 0;
                ...
                if (req.query.TOURNAMENT_ID[0] == "T") {
                  pl1sum +=
                    result4.MATCHES[matchid].PLAYER1_SCORE.set4 +
                    result4.MATCHES[matchid].PLAYER1_SCORE.set5;
                  pl2sum +=
                    result4.MATCHES[matchid].PLAYER2_SCORE.set4 +
                    result4.MATCHES[matchid].PLAYER2_SCORE.set5;
                }
                if (pl1sum > pl2sum) {
                                  ...
                                  if (re2) {
                                    USER.updateOne(
                                      ...
                                      function (er3, re3) {
                                        ...
                                        if (re3) {
                                          console.log(
                                            "Updating Next Match Spot"
                                          );
                                          if (
                                            result4.MATCHES[matchid]
                                            ...
                                            matchesmodel.updateOne(
                                              ...
                                              {
                                                $set: {
                                                  "MATCHES.$[elem].PLAYER1":
                                                    WINNER_ID,
                                                  "MATCHES.$[elem2].winner_id":
                                                    WINNER_ID,
                                                  "MATCHES.$[elem2].winner_name":
                                                    winner_name.NAME,
                                                  "MATCHES.$[elem2].completion_status":
                                                    "Done",
                                                },
                                              },
                                              {
              ...
              function (error, result) {
                if (error) {
                  ...
                  for (var i = 0; i < result2.NO_OF_KNOCKOUT_ROUNDS - 1; i++) {
                    if (
                      result.MATCHES[i].completion_status == "Not Complete" &&
                      !(
                        (result.MATCHES[i].PLAYER1 == "Not Booked" ||
                          result.MATCHES[i].PLAYER1 == "Not Yet Assigned") &&
                        (result.MATCHES[i].PLAYER2 == "Not Booked" ||
                          result.MATCHES[i].PLAYER2 == "Not Yet Assigned")
                      )
                    ) {
                      var us1 = "";
                      var us2 = "";
                ...
                if (error) {
                  ...
                  for (var i = 0; i < result2.NO_OF_KNOCKOUT_ROUNDS - 1; i++) {
                    if (
                      ...
                      for (let k = 0; k < allUsers.length; k++) {
                        if (
                          `${dbles_partner_1_id}` == `${allUsers[k].USERID}`
                        ) {
                          us3 = allUsers[k].NAME;
                          break;
                        }
                      ...
                      for (let k = 0; k < allUsers.length; k++) {
                        if (
                          `${dbles_partner_2_id}` == `${allUsers[k].USERID}`
                        ) {
                          us4 = allUsers[k].NAME;
                          break;
                        }
    ...
    function (error, result) {
      if ((error, result)) {
        if (error) {
          ...
          var spotbooked =
            result.SPOT_STATUS_ARRAY.indexOf(`confirmed-${req.query.USERID}`) +
            1;
          USER.findOne(
            {
              USERID: req.query.USERID,
    ...
    if (r1 && result1) {
      if (r1) {
        ...
        for (let i = 0; r1[i]; i++) {
          ...
          const temp_start_time = new Date(
            start_time.getTime() -
              r1[i].REGISTRATION_CLOSES_BEFORE * 60 * 60 * 1000
          );
          ...
          //console.log(curDate.getTime()>end_date.getTime())
          if (
            curDate.getTime() < temp_start_time.getTime() &&
            r1[i].REGISTRATION_CLOSES_BEFORE > 0
          ) {
            //console.log(r1[i])
    ...
    function (err, rr) {
      if (err) {
        ...
        const start_time = new Date(
          new Date(rr.START_TIMESTAMP).getTime() + hrs * 60 * 60 * 1000
        ).toISOString();
        tournamentModel.updateOne(
          {
            TOURNAMENT_ID: tid,
